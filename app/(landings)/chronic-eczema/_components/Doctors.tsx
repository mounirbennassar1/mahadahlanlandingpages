    ...
    <div className="relative overflow-hidden">
      ...
      >
        ...
        <div className="relative w-[clamp(240px,32vw,330px)] shrink-0">
          <div
            className="pointer-events-none absolute -inset-x-3.5 -top-3.5 bottom-6 rounded-t-full border border-[var(--color-che-line-gold)]"
            aria-hidden
          />
          <div className="relative aspect-[3/4] overflow-hidden rounded-t-full bg-[var(--color-che-card)]">
            <Image
              src={d.image}
              alt={d.imageAlt}
              fill
          ...
          <figcaption className="mt-3.5 flex items-start gap-3 border-t border-[var(--color-che-line)] pt-2.5">
            <span
              className="mt-[9px] h-px w-7 shrink-0 bg-[var(--color-che-gold)]"
              aria-hidden
            />
        ...
        <div className="min-w-[300px] flex-1 text-center md:max-w-[560px] md:text-right">
          <span className="text-[0.72rem] font-extrabold tracking-[0.22em] text-[var(--color-che-gold-bright)]">
            {d.label}
          </span>
          <h3 className="mt-2 mb-0 text-3xl font-extrabold sm:text-4xl">
            {d.name}
          </h3>
          <p className="mt-1.5 font-bold text-[var(--color-che-ink-2)]">
            {d.title}
          </p>
          ...
          <ul className="mt-7 list-none p-0 text-right">
            {d.credentials.map((c) => (
              <li
                key={c}
                className="flex items-start gap-3.5 border-t border-[var(--color-che-line)] py-3"
              >
                <Icon.BadgeCheck className="mt-1 size-[18px] shrink-0 text-[var(--color-che-gold)]" />
                <span className="text-[0.9rem] leading-7 font-bold text-[var(--color-che-ink-2)]">
                  {c}
                </span>
              </li>
            ))}
            <li className="h-px bg-[var(--color-che-line)]" aria-hidden />
          </ul>
        </div>
      </motion.div>
      ...
      <div className="mt-10 flex items-center justify-center gap-5">
        <button
          ...
          className="flex size-11 cursor-pointer items-center justify-center rounded-full border border-[var(--color-che-line-strong)] bg-transparent text-[var(--color-che-ink)] transition-colors hover:border-[var(--color-che-gold-bright)] hover:text-[var(--color-che-gold-bright)]"
        >
          ...
          {DOCTORS.map((doc, i) => (
            <button
              ...
              className={`h-1 cursor-pointer transition-all duration-300 ${
                i === index
                  ? "w-9 bg-[var(--color-che-gold)]"
                  : "w-4 bg-[var(--color-che-line-strong)] hover:bg-[var(--color-che-gold)]"
              }`}
            />
          ))}
        </div>
