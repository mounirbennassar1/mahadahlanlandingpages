...
/** Icons for the "why us" cards, in content order. */
const FEATURE_ICONS = [User, Sparkles, Clock] as const;

/** Portraits for the doctor cards, in content order. */
const DOCTOR_IMAGES = [
  "/acne/team/dr-maha.avif",
  "/acne/team/dr-inas.avif",
  "/acne/team/dr-lajin.avif",
] as const;

/** Counter config for the hero stat strip, in content order. The values feed
 *  the `data-count` animation, so they live here rather than in the CMS. */
const HERO_STATS: { v: string; suffix: string; isCount?: boolean }[] = [
  { v: "4.8", suffix: "★" },
  { v: "1158", suffix: "+", isCount: true },
  { v: "20", suffix: "+", isCount: true },
];

/** Anchors for the footer quick links, in content order. */
const FOOTER_LINKS = ["#doctors", "#top", "whatsapp"] as const;

/* Genuine quotes from the clinic's public Google reviews — not editable copy,
   so they stay in code; only the section heading around them is in the CMS. */
const reviews = [
  {
    name: "Fatima Bushra",
...
export function Landing({ content }: { content: ContentOf<typeof ACNE> }) {
  const marqueeWords = content.marquee.items.flatMap((word) => [word, "•"]);
  const features = content.why.cards.map((card, i) => ({
    ...card,
  ...
  const heroStats = content.hero.stats.map((stat, i) => ({
    ...stat,
    ...HERO_STATS[i],
  }));
  ...
  return (
    ...
    >
      ...
      <MobileStickyCTA whatsappHref={WHATSAPP} book={content.sticky.book} />
              ...
              >
                ...
                <div className="text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-stone-900 ms-1">{content.hero.rating}</span>
                  </div>
                  <div className="text-xs text-stone-500 leading-tight mt-0.5">
                    {content.hero.ratingNote}
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-stone-900 text-sm sm:text-base leading-tight">
                      {content.hero.resultTitle}
                    </div>
                    <div className="text-xs text-stone-500 mt-0.5">
                      {content.hero.resultSub}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="relative py-20 sm:py-28 lg:py-32">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-14 sm:mb-20">
            ...
            >
              {content.why.title}{" "}
              <span className="text-gold-700">{content.why.highlight}</span>
            </h2>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
            ...
            <h2
              ...
              className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-stone-900 tracking-tight"
            >
              {content.reviews.titlePre}{" "}
              <span data-count="1158" className="text-gold-700">
                0
              </span>{" "}
              {content.reviews.titlePost}
            </h2>
            <div
              data-reveal
      ...
      <footer className="bg-stone-950 text-stone-400 pt-16 sm:pt-20 pb-28 md:pb-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-12 mb-14">
            ...
            <div>
              <h5 className="font-bold mb-5 text-white">{content.footer.linksTitle}</h5>
              <ul className="space-y-3 text-sm">
                {content.footer.links.map((link, i) => {
                  const target = FOOTER_LINKS[i];
                  const external = target === "whatsapp";
                  return (
                    <li key={target}>
                      <a
                        href={external ? WHATSAPP : target}
            ...
            <div>
              <h5 className="font-bold mb-5 text-white">{content.footer.contactTitle}</h5>
              <ul className="space-y-3 text-sm">
            ...
            <div>
              <h5 className="font-bold mb-5 text-white">{content.footer.hoursTitle}</h5>
              <ul className="space-y-3 text-sm">
                {content.footer.hours.map((row) => (
                  <li key={row.days} className="flex justify-between gap-2">
                    <span>{row.days}</span>
                    <span className="text-stone-300">{row.time}</span>
                  </li>
                ))}
              </ul>
            </div>
