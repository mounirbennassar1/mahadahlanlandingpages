...
/* ─── Image + icon for each service card, in content order ─── */
...
/* ─── Icon paths for the reassurance strip under the form, in content order ─── */
...
/* ─── Icon paths for the clinic contact lines, in content order ─── */
const CLINIC_ICONS = [
...
function ServiceCard({
  ...
  title: string; desc: string; cta: string; delay: number
}) {
  return (
    ...
    <>
      ...
      >
        ...
        <div className="hidden lg:flex gap-10 text-xs font-semibold text-gray-700">
          {c.nav.links.map(({ label }, i) => (
            <motion.a key={NAV_HREFS[i]} href={NAV_HREFS[i]} className="hover:text-gold-400 transition-colors" whileHover={{ y: -2 }}>
              {label}
            </motion.a>
          ))}
        </div>
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-16 pt-24 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-0 items-center min-h-screen">
            ...
            <div className="flex items-center lg:pl-12">
              ...
              >
                ...
                >
                  {c.hero.title}<br />
                  <span className="italic font-light text-white/70">{c.hero.titleDots}</span>{' '}
                  <span className="font-bold">{c.hero.titleStrong}</span>
                </motion.h1>
            ...
            <div className="relative hidden lg:flex items-center justify-start h-full">
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-serif font-bold text-charcoal text-sm leading-none">{c.hero.badgeCertTitle}</p>
                  <p className="text-[11px] text-gray-500 mt-0.5">{c.hero.badgeCertBody}</p>
                </div>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-3 mb-2 flex-row-reverse">
                  ...
                  <span className="text-[11px] text-gray-500 font-semibold">{c.hero.badgeSlotStatus}</span>
                </div>
                <p className="font-serif text-charcoal text-sm font-medium text-right">
                  {c.hero.badgeSlotLabel} <span className="text-gold-500">{c.hero.badgeSlotValue}</span>
                </p>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-charcoal leading-none">{c.hero.badgeLastTitle}</p>
                    <p className="text-[11px] text-gold-500 mt-0.5">{c.hero.badgeLastValue}</p>
                  </div>
                </div>
              </motion.div>
            </div>
        ...
        <motion.div
          ...
          transition={{ delay: 2 }}
        >
          <span className="text-[10px] text-white/50">{c.hero.scroll}</span>
          <motion.div
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            <Reveal className="w-full lg:w-1/2 relative" delay={0}>
              ...
              <Reveal delay={0.3} className="absolute -bottom-10 -left-10 bg-white p-6 rounded-2xl shadow-xl z-20 w-64 hidden md:block">
                <h4 className="font-script text-3xl text-gold-400 mb-2">{c.about.philosophyTitle}</h4>
                <p className="text-xs text-gray-500 leading-loose">
                  {c.about.philosophyBody}
                </p>
              </Reveal>
            </Reveal>

            <Reveal className="w-full lg:w-1/2" delay={0.2}>
              <h2 className="font-script text-4xl text-shimmer-gold mb-2">{c.about.script}</h2>
              <h3 className="font-serif text-4xl md:text-5xl text-charcoal mb-8">
                {c.about.title} <span className="text-gradient-gold">MD Clinics</span>
              </h3>
      ...
      <section id="services" className="py-24 lg:py-32 bg-blush-50 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 lg:px-16 max-w-7xl">
          <Reveal className="text-center mb-20">
            <h2 className="font-script text-4xl text-shimmer-gold mb-2">{c.services.script}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 items-stretch">
            {c.services.cards.map((service, i) => (
              <ServiceCard
                key={service.title}
                img={SERVICE_VISUALS[i].img}
                alt={SERVICE_VISUALS[i].alt}
                delay={SERVICE_VISUALS[i].delay}
                title={service.title}
                desc={service.desc}
                cta={c.services.cardCta}
                icon={SERVICE_VISUALS[i].icon}
              />
            ))}
          </div>
        </div>
      ...
      <section id="reviews" className="py-24 bg-white border-t border-blush-200">
        <div className="container mx-auto px-6 lg:px-16 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16">
            <Reveal>
              <h2 className="font-script text-4xl text-shimmer-gold mb-2">{c.reviews.script}</h2>
              <h3 className="font-serif text-4xl text-charcoal">{c.reviews.title}</h3>
            </Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {c.reviews.cards.map(({ quote, initial, name, service }, i) => (
        ...
        <div className="container mx-auto px-6 lg:px-16 max-w-7xl relative z-10">
          <Reveal className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <span className="text-xs text-gold-400/80 font-semibold">{c.booking.eyebrow}</span>
              <span className="w-8 h-px bg-gradient-to-r from-transparent to-gold-400" />
            </div>
            <h2 className="font-script text-5xl text-shimmer-gold mb-2">{c.booking.script}</h2>
            <h3 className="font-serif text-3xl md:text-4xl text-white/90">{c.booking.title}</h3>
          </Reveal>
                    ...
                    {c.booking.assurances.map(({ label }, i) => (
                      <div key={label} className="flex items-center gap-2 text-white/50 text-xs">
                        <svg className="w-4 h-4 text-gold-400/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={ASSURANCE_ICONS[i]} />
                        </svg>
                        {label}
                      </div>
                    ))}
            ...
            <Reveal className="lg:col-span-2" delay={0.25}>
              ...
              >
                <div className="p-8 space-y-5">
                  <h4 className="font-serif text-xl text-white font-semibold">{c.booking.clinicTitle}</h4>
                  {c.booking.clinicLines.map(({ text }, i) => (
                    <div key={text} className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5"
                        style={{ background: 'rgba(212,175,55,0.15)' }}>
                        <svg className="w-4 h-4 text-gold-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={CLINIC_ICONS[i]} />
                        </svg>
                      </div>
      ...
      <footer className="bg-charcoal pt-16 pb-8 border-t border-gold-400/30 text-white">
        <div className="container mx-auto px-6 lg:px-16 text-center">
          <Reveal>
            <div className="font-script text-5xl text-shimmer-gold mb-6">{c.footer.brand}</div>
          ...
          <div className="border-t border-gold-400/30 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500">
            ...
            <div className="flex gap-4 mt-4 md:mt-0">
              {c.footer.links.map((link) => (
                <motion.a key={link} href="#" className="hover:text-gold-400 transition-colors" whileHover={{ y: -1 }}>{link}</motion.a>
              ))}
            </div>
          </div>
        </div>
