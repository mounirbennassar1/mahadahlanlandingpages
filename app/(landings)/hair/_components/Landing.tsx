...
/** Social networks: URLs and glyphs stay in code, labels come from content. */
const SOCIAL_ASSETS = [
  {
...
const WA =
  "https://wa.me/966503377702?text=السلام%20عليكم%20ورحمة%20الله%20وبركاته%0Aعندي%20استفسار%20بخصوص%20خدماتكم%20للشعر";
...
function BeforeAfterSlider({
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-[#c9a84c] px-4 py-1.5 rounded-full text-sm font-bold z-10 border border-[#c9a84c]/20">{afterLabel}</div>
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden" style={{ clipPath: `inset(0 0 0 ${position}%)` }}>
        ...
        <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md text-white px-4 py-1.5 rounded-full text-sm font-bold z-10 border border-white/20">{beforeLabel}</div>
      </div>
...
/** Real Google reviews — kept in code so they cannot be edited. */
...
/** Material icon per feature card, in content order. */
const FEATURE_ICONS = ["biotech", "medication", "assignment_ind"] as const;

/** Portraits for the medical team, in content order. */
const TEAM_IMAGES = [
  "/hair/team/dr-maha.avif",
  "/hair/team/dr-inas.avif",
  "/hair/team/dr-lajin.avif",
] as const;

/** Footer quick-link destinations, in content order. */
const FOOTER_QUICK_HREFS = ["#services", "#results", "#about"] as const;

export function Landing({ content }: { content: ContentOf<typeof HAIR> }) {
  const c = content;
  const socialLinks = SOCIAL_ASSETS.map((asset, i) => ({
    ...asset,
    name: c.social.links[i]?.label ?? "",
  }));
  const features = c.features.cards.map((card, i) => ({
    ...card,
    icon: FEATURE_ICONS[i],
  }));
  const doctors = c.team.people.map((person, i) => ({
    ...person,
    img: TEAM_IMAGES[i],
  }));
    ...
    <div className="min-h-screen bg-[#f9f7f2] text-[#1a3a2a]">
      ...
      <main>
          ...
          >
            <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
              ...
              <motion.h2 variants={fadeUp} custom={0.05} className="text-3xl md:text-4xl lg:text-5xl font-bold leading-[1.3] text-[#1a3a2a]">
                ...
                <span className="relative inline-block">
                  <span className="text-[#c9a84c]">{c.hero.titleHighlight}</span>
                  <motion.span
              ...
              <motion.div variants={fadeUp} custom={0.1} className="text-base text-slate-600 max-w-xl leading-relaxed space-y-4">
                ...
                <div className="bg-white/60 border border-white/50 p-4 rounded-2xl shadow-sm">
                  <h4 className="font-bold text-[#1a3a2a] mb-1 flex items-center gap-2">
                    <span className="text-[#c9a84c]">🔹</span> {c.hero.typeOneTitle}
                  </h4>
                  <p className="text-sm">{c.hero.typeOneBody}</p>
                  <p className="text-sm mt-2">
                    <span className="font-semibold text-[#1a3a2a]">{c.hero.typeOneCausesLabel}</span> {c.hero.typeOneCauses} <br />
                    <span className="font-semibold text-[#c9a84c]">{c.hero.typeOneNote}</span>
                  </p>
                </div>

                <div className="bg-white/60 border border-white/50 p-4 rounded-2xl shadow-sm">
                  <h4 className="font-bold text-[#1a3a2a] mb-1 flex items-center gap-2">
                    <span className="text-[#c9a84c]">🔹</span> {c.hero.typeTwoTitle}
                  </h4>
                  <p className="text-sm">{c.hero.typeTwoBody} <br />
                    <span className="font-semibold text-[#c9a84c]">{c.hero.typeTwoNote}</span>
                  </p>
                </div>
              ...
              <motion.div variants={fadeUp} custom={0.15} className="flex flex-col sm:flex-row flex-wrap gap-4 pt-2">
                ...
                <motion.a
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.97 }}
                  className="w-full sm:w-auto bg-[#25D366] text-white px-8 py-4 rounded-xl font-bold text-lg shadow-lg shadow-[#25D366]/25 flex items-center justify-center gap-3 text-center hover:bg-[#1EBE5D] transition-colors"
                >
            ...
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} className="relative">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.8, duration: 0.5, type: "spring", bounce: 0.4 }} className="absolute -bottom-6 -left-6 bg-white rounded-2xl p-4 shadow-xl border border-[#1a3a2a]/5 flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-[#1a3a2a] text-sm">{c.hero.badgeTitle}</div>
                  <div className="text-xs text-slate-400">{c.hero.badgeSub}</div>
                </div>
              </motion.div>
            </motion.div>
          </motion.div>
        ...
        <section id="gallery" className="py-24 bg-[#1a3a2a] relative overflow-hidden border-t border-white/5">
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="text-center mb-16">
              <motion.h3 variants={fadeUp} className="text-[#c9a84c] font-bold mb-4 uppercase text-sm tracking-widest">{c.gallery.eyebrow}</motion.h3>
              <motion.p variants={fadeUp} custom={0.05} className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-6">{c.gallery.title}</motion.p>
              ...
              {galleryPairs.map((pair, index) => (
                <motion.div key={index} variants={fadeUp} custom={index * 0.1}>
                  <BeforeAfterSlider
                    before={pair.before}
                    after={pair.after}
                    beforeLabel={c.gallery.beforeLabel}
                    afterLabel={c.gallery.afterLabel}
                  />
                </motion.div>
              ))}
            </motion.div>
        ...
        <section className="py-24 bg-white relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="text-center max-w-3xl mx-auto mb-16">
              <motion.h3 variants={fadeUp} className="text-[#c9a84c] font-bold mb-4 uppercase text-sm tracking-widest">{c.team.eyebrow}</motion.h3>
              <motion.p variants={fadeUp} custom={0.05} className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#1a3a2a] mb-6 leading-relaxed">{c.team.title}</motion.p>
              <motion.p variants={fadeUp} custom={0.1} className="text-slate-600">{c.team.sub}</motion.p>
            </motion.div>
            ...
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={staggerContainer} className="space-y-8">
                <motion.div variants={fadeUp} className="space-y-4">
                  <h3 className="text-[#c9a84c] font-bold">{c.treatments.eyebrow}</h3>
                  <h2 className="text-3xl md:text-4xl font-bold text-[#1a3a2a] leading-tight">{c.treatments.title}</h2>
                </motion.div>

                <div className="space-y-6">
                  {c.treatments.cards.map((item, i) => (
        ...
        <section id="results" className="py-16 md:py-24 bg-[#c9a84c]/5 overflow-hidden">
          <div className="max-w-7xl mx-auto px-6">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={staggerContainer} className="text-center max-w-3xl mx-auto mb-12">
              <motion.h3 variants={fadeUp} className="text-[#c9a84c] font-bold mb-4 uppercase text-sm tracking-widest">{c.reviews.eyebrow}</motion.h3>
              <motion.p variants={fadeUp} custom={0.05} className="text-2xl md:text-3xl font-bold text-[#1a3a2a] mb-6">{c.reviews.title}</motion.p>
              ...
              <motion.div variants={fadeUp} custom={0.1} className="inline-flex items-center gap-3 bg-white rounded-full px-6 py-3 shadow-md border border-[#1a3a2a]/5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-bold text-[#1a3a2a]">{c.reviews.rating}</span>
                ...
                <span className="text-sm text-slate-500">{c.reviews.count}</span>
              </motion.div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 60, scale: 0.98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="bg-[#1a3a2a] rounded-[3rem] p-6 md:p-10 lg:p-16 relative overflow-hidden">
              ...
              <div className="relative z-10 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                <div className="space-y-6 text-center lg:text-right">
                  ...
                  <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight">{c.booking.title}</motion.h2>
                  <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="text-slate-300 text-lg leading-relaxed">{c.booking.body}</motion.p>
      ...
      <section className="bg-white py-12 border-t border-[#1a3a2a]/10">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h4 className="text-[#1a3a2a] font-bold text-lg mb-8">{c.social.title}</h4>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="space-y-6">
              ...
              <p className="text-slate-500 text-sm leading-relaxed">{c.footer.about}</p>
            ...
            <div>
              <h5 className="font-bold mb-6 text-[#1a3a2a]">{c.footer.quickTitle}</h5>
              <ul className="space-y-4 text-sm text-slate-500">
                {footerQuick.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="hover:text-[#c9a84c] transition-colors">{link.label}</a>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div>
              <h5 className="font-bold mb-6 text-[#1a3a2a]">{c.footer.contactTitle}</h5>
              <ul className="space-y-4 text-sm text-slate-500">
            ...
            <div>
              <h5 className="font-bold mb-6 text-[#1a3a2a]">{c.footer.newsletterTitle}</h5>
              <p className="text-xs text-slate-500 mb-4">{c.footer.newsletterBody}</p>
              <form className="flex gap-2" onSubmit={(e) => e.preventDefault()}>
                <input className="flex-1 bg-white border border-[#1a3a2a]/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]/40 focus:border-[#c9a84c]" placeholder={c.footer.newsletterPlaceholder} type="email" />
          ...
          <div className="pt-8 border-t border-[#1a3a2a]/5 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-slate-400">{c.footer.copyright}</p>
            <div className="flex gap-6 text-xs text-slate-400">
              <a href="#" className="hover:text-[#c9a84c] transition-colors">{c.footer.privacy}</a>
              <a href="#" className="hover:text-[#c9a84c] transition-colors">{c.footer.terms}</a>
            </div>
          </div>
        </div>
      </footer>
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.94 }}
        className="fixed bottom-6 right-6 z-50 flex size-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-2xl shadow-[#25D366]/30"
      >
        <span aria-hidden className="absolute -inset-1 animate-ping rounded-full bg-[#25D366]/30" />
