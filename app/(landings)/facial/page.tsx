...
import { CheckoutProvider, PayButton } from "@/components/checkout";
...
export default async function FacialLanding() {
  const [c, items] = await Promise.all([getPageContent(FACIAL), getSellableItems(FACIAL.slug)]);

  return (
    <CheckoutProvider
      items={items}
      page={{ slug: FACIAL.slug, title: FACIAL.title, path: FACIAL.path }}
      whatsappTopic="عندي استفسار بخصوص أنواع تنظيف البشرة عندكم"
    >
      ...
      <section id="booking" className="relative py-32 overflow-hidden bg-[#050505]">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <PayButton className="group relative inline-flex items-center justify-center gap-2.5 px-10 py-4 bg-[#D4AF37] text-black font-bold text-lg overflow-hidden transition-all hover:scale-105 active:scale-95 rounded-2xl cursor-pointer">
              ...
              <span className="relative z-10">{c.booking.pay}</span>
